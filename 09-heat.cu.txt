...
int main()
{
  ...
  for( int i = 0; i  ninj; ++i) {
    temp1_ref[i] = temp2_ref[i] = temp1[i] = temp2[i] = (float)rand()(float)(RAND_MAX100.0f);
  }
  ...
  dim3 grid((njtblocks.x)+1, (nitblocks.y)+1, 1);
  ...
  for (istep=0; istep  nstep; istep++) {
    ...
    if (ierrSync != cudaSuccess) { printf(Sync error %sn, cudaGetErrorString(ierrSync)); }
    if (ierrAsync != cudaSuccess) { printf(Async error %sn, cudaGetErrorString(ierrAsync)); }
  ...
  if (maxError  0.0005f)
    printf(Problem! The Max Error of %.5f is NOT within acceptable bounds.n, maxError);
  else
    printf(The Max Error of %.5f is within acceptable bounds.n, maxError);
